...
class test_tools():
    ...
    def test_matrix_size(self):
        nt.assert_equal(tools.sidereal_converter('test_data.dat').shape, (27,3))

    def test_year(self):
        # See if the year is before 1973. 
        data = tools.sidereal_converter('test_data.dat')
        cols = []
        for i in range(0,27):
            cols_element = data[i][0]
            cols.append(cols_element)        
        cols_list = list(cols)
        w = all(i >= 0 for i in cols_list)
        nt.assert_equal(w, True) 
